...
class SystemProxyNotifier extends StateNotifier<SystemProxyState> {
  ...
  Future<void> enableSystemProxy() async {
    ...
    try {
      ...
      appLogger.info('Current state: isEnabled=${state.isEnabled}, isLoading=${state.isLoading}');
      ...
      try {
        appLogger.info('Calling api.setSystemProxy with httpPort=${state.httpPort}, socksPort=${state.socksPort}');
        api.setSystemProxy(
  ...
  Future<void> setProxyPorts({int? httpPort, int? socksPort}) async {
    try {
      ...
      appLogger.info('Proxy ports updated: HTTP=$newHttpPort, SOCKS=$newSocksPort');
